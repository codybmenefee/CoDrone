...
def generate_report_from_template(
    ...
    Args:
        template_id: ID of the template to use (e.g., 'crop-health-basic', 'volume-basic', 'area-survey-basic')
    ...
    try:
        ...
        result = {
            ...
            "data_included": {
                ...
                "measurements": len([k for k in data_context.get("analysisResults", {}).keys() if data_context["analysisResults"][k] is not None]),
                "visualizations": len(data_context.get("visualizations", {}))
            },
            "preview": f"Report '{title}' has been generated using the {template_info['name']} template. The report includes analysis data from {data_context['metadata'].get('location', 'the surveyed area')} collected on {data_context['metadata'].get('date')}.",
            "next_steps": [
...
def create_custom_report(
    ...
    try:
        ...
        valid_types = ["crop_health", "volume_measurement", "area_survey", "inspection", "environmental"]
        ...
        report_structure = generate_report_structure(report_type, visualizations, detail_level)
        ...
        return json.dumps({
            "success": False,
            "error": f"Failed to create custom report: {str(e)}"
        }, indent=2)
        ...
        templates = [
            {
                ...
                "use_cases": ["NDVI monitoring", "Vegetation health assessment", "Crop stress detection"],
            ...
            },
            {
                ...
                "components": ["executive-summary", "drone-map", "analysis-chart", "measurement-table", "key-findings", "zone-analysis", "recommendations"],
                "use_cases": ["Detailed crop monitoring", "Precision agriculture", "Treatment planning"],
            ...
            },
            {
                ...
                "use_cases": ["Stockpile measurement", "Material volume tracking", "Inventory management"],
                "estimated_time": "2-3 minutes to generate"
            },
            {
            ...
            },
            {
                ...
                "components": ["executive-summary", "drone-map", "analysis-chart", "key-findings"],
                "use_cases": ["Infrastructure monitoring", "Defect detection", "Maintenance planning"],
                "estimated_time": "3-4 minutes to generate"
            },
            {
                ...
                "components": ["executive-summary", "drone-map", "analysis-chart", "measurement-table", "key-findings"],
                "use_cases": ["Environmental monitoring", "Impact assessment", "Compliance reporting"],
                "estimated_time": "3-4 minutes to generate"
            }
        ]
...
def get_template_info(template_id: str) -> Dict[str, str]:
    ...
    templates = {
        "crop-health-basic": {"name": "Basic Crop Health Analysis", "category": "crop-health"},
        "crop-health-detailed": {"name": "Detailed Crop Health Report", "category": "crop-health"},
        "volume-basic": {"name": "Volume Measurement Report", "category": "volume-measurement"},
        "area-survey-basic": {"name": "Area Survey Report", "category": "area-survey"},
        "inspection-basic": {"name": "Infrastructure Inspection", "category": "inspection"},
        "environmental-basic": {"name": "Environmental Monitoring", "category": "environmental"}
    }
    return templates.get(template_id, {"name": "Unknown Template", "category": "unknown"})

def generate_report_structure(report_type: str, visualizations: List[str], detail_level: str) -> Dict[str, Any]:
    """Generate report structure based on type and requirements."""
    base_structure = {
        "header": {"title": True, "metadata": True},
        "sections": []
    }
...
def analyze_data_for_insights(analysis_data: str, report_type: str) -> Dict[str, Any]:
    ...
    if data_context["metadata"]:
        ...
        if "resolution" in data_context["metadata"]:
            insights["key_points"].append(f"Excellent resolution: {data_context['metadata']['resolution']}")
    
    return insights

def generate_ai_recommendations(insights: Dict[str, Any], report_type: str) -> List[str]:
...
def select_optimal_template(insights: Dict[str, Any], report_type: str, preferences: Dict[str, Any]) -> Dict[str, str]:
    ...
    if preferences.get("detailLevel") == "comprehensive" and template_id == "crop-health-basic":
...
def generate_content_suggestions(insights: Dict[str, Any], report_type: str) -> Dict[str, List[str]]:
...
def summarize_data_context(data_context: Dict[str, Any]) -> str:
    ...
    return " | ".join(summary_parts) if summary_parts else "Basic analysis data available"
...
async def generate_report_from_ai_analysis(analysis_data: str, report_type: str, preferences: Dict[str, Any]) -> Dict[str, Any]:
