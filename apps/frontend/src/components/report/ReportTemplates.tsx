...
import { FileText, Crop, Mountain, BarChart, Camera, ArrowRight, Eye } from 'lucide-react';
import { ReportTemplate, ReportTemplateCategory, DroneDataContext } from '../../types/report';
...
const ReportTemplates: React.FC<ReportTemplatesProps> = ({
  ...
  const categories: ReportTemplateCategory[] = [
    { id: 'all', name: 'All Templates', description: 'View all available templates', icon: '📄' },
    { id: 'crop-health', name: 'Crop Health', description: 'NDVI and vegetation analysis', icon: '🌱' },
    { id: 'volume-measurement', name: 'Volume Measurement', description: 'Stockpile and volume calculations', icon: '📊' },
    { id: 'area-survey', name: 'Area Survey', description: 'Land area and boundary analysis', icon: '🗺️' },
    { id: 'inspection', name: 'Inspection', description: 'Infrastructure and asset inspection', icon: '🏗️' },
    { id: 'environmental', name: 'Environmental', description: 'Environmental monitoring reports', icon: '🌍' }
  ];
  ...
  const getCategoryIcon = (categoryId: string) => {
    switch (categoryId) {
      case 'crop-health': return <Crop className="w-5 h-5" />;
      case 'volume-measurement': return <BarChart className="w-5 h-5" />;
      case 'area-survey': return <Mountain className="w-5 h-5" />;
      case 'inspection': return <Camera className="w-5 h-5" />;
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Choose Report Template</h1>
        <p className="text-gray-600">
          Select a template that best fits your drone data analysis needs. Templates can be customized after selection.
        </p>
      </div>
        ...
        {filteredTemplates.map(template => (
          ...
          >
            <div className="p-6">
              ...
              <div className="mb-4">
                <p className="text-xs text-gray-500 mb-2">Components included:</p>
          ...
          <div 
            ...
            dangerouslySetInnerHTML={{ 
              __html: template.html_template.replace(/\${.*?}/g, (match) => {
                ...
                if (match.includes('location')) return dataContext?.metadata?.location || 'Sample Location';
                if (match.includes('date')) return dataContext?.metadata?.date || new Date().toLocaleDateString();
                if (match.includes('area')) return dataContext?.analysisResults?.area?.toFixed(1) || '45.7';
                return 'Sample Data';
              })
            }}
          />
