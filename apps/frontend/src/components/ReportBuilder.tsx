...
import { Save, Download, Eye, EyeOff, FileText, Settings } from 'lucide-react';
import { 
  ReportBuilderProps, 
  DroneDataContext, 
  ExportOptions 
...
const ReportBuilder: React.FC<ReportBuilderProps> = ({
  ...
  useEffect(() => {
    ...
    droneComponentCategories.forEach(category => {
      blockManager.add(category.id, {
        ...
        category: { id: category.id, label: category.label, open: category.open }
      });
    });
  ...
  const exportToPDF = async (html: string, css: string, options: ExportOptions) => {
  ...
  const exportToHTML = async (html: string, css: string, options: ExportOptions) => {
    const fullHTML = `
      <!DOCTYPE html>
      <html lang="en">
      <head>
