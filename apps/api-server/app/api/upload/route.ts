...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const file of files) {
      ...
      // Validate file type
      const allowedExtensions = ['.tif', '.tiff', '.jpg', '.jpeg', '.png', '.laz', '.las', '.ply'];
      ...
      if (fileExtension === '.tif' || fileExtension === '.tiff') {
        fileType = file.name.toLowerCase().includes('dsm') ? 'dsm' : 'orthomosaic';
      } else if (fileExtension === '.laz' || fileExtension === '.las' || fileExtension === '.ply') {
        fileType = 'pointcloud';
      }
