...
export const AreaCalculationSchema = z.object({
  polygonCoordinates: z.string().describe('GeoJSON polygon as string'),
  coordinateSystem: z.string().default('EPSG:4326').describe('Coordinate system'),
...
export const FileProcessingSchema = z.object({
  ...
  processingType: z.enum(['orthomosaic', 'dsm', 'pointcloud']).describe('Type of processing'),
  outputPath: z.string().optional().describe('Output directory path'),
  parameters: z.record(z.unknown()).optional().describe('Additional processing parameters'),
});
