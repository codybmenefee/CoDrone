...
FastAPI endpoints for report template management, report generation,
and export functionality. Integrates with the AI agent for automated
report generation.
"""
...
async def update_template(
    template_id: str, request: CreateTemplateRequest
) -> ReportTemplate:
...
async def generate_report(request: ReportGenerationRequest) -> ReportGenerationResponse:
    ...
    return ReportGenerationResponse(
        id=report_id, html=html, css=css, createdAt=report_data["created_at"]
    )
...
async def generate_ai_report(request: AIReportRequest) -> ReportGenerationResponse:
    ...
    try:
        ...
        ai_result = await generate_report_from_ai_analysis(
            request.analysisData, request.reportType, request.preferences
        )
        ...
        if basic_template_id and basic_template_id in templates_store:
            ...
            result = await generate_report(fallback_request)
            return ReportGenerationResponse(**result.dict())
...
async def save_report(report_id: str, request: SaveReportRequest) -> Dict[str, Any]:
...
async def list_reports(
    ...
    if search:
        search_lower = search.lower()
        reports = [r for r in reports if search_lower in r.get("title", "").lower()]

    if category:
        reports = [r for r in reports if r.get("template_id", "").startswith(category)]
...
async def export_report_to_pdf(
    report_id: str, options: Optional[ExportOptions] = None
) -> Dict[str, Any]:
    ...
    if not PDF_SUPPORT:
        raise HTTPException(
            status_code=501,
            detail=(
                "PDF export not available. Install weasyprint for "
    ...
    try:
        ...
        full_html = create_full_html_document(
            report["html"], report["css"], options.dict()
        )
...
async def convert_html_to_pdf(
    html: str, css: str, options: Optional[ExportOptions] = None
) -> Dict[str, Any]:
    ...
    if not PDF_SUPPORT:
        raise HTTPException(
            status_code=501,
            detail=(
                "PDF conversion not available. Install weasyprint for "
                "server-side PDF generation."
            ),
        )
...
async def get_template_categories() -> List[Dict[str, Any]]:
    ...
    for template in templates_store.values():
        ...
        current_count = categories[category]["template_count"]
        if isinstance(current_count, int):
            categories[category]["template_count"] = current_count + 1
        else:
    ...
    def replace_var(match):
        ...
        try:
            ...
            if "metadata?.location" in var_expr:
                return (
                    context.metadata.get("location", "Unknown Location")
                    if context.metadata
                    else "Unknown Location"
                )
            elif "metadata?.date" in var_expr:
                return (
                    context.metadata.get("date", datetime.now().strftime("%Y-%m-%d"))
                    if context.metadata
                    else datetime.now().strftime("%Y-%m-%d")
                )
            elif "analysisResults?.area" in var_expr:
                return (
                    str(context.analysisResults.get("area", 0))
                    if context.analysisResults
                    else "0"
                )
            elif "metadata?.resolution" in var_expr:
                return (
                    context.metadata.get("resolution", "2.3 cm/pixel")
                    if context.metadata
                    else "2.3 cm/pixel"
                )
            elif "metadata?.imageCount" in var_expr:
                return (
                    str(context.metadata.get("imageCount", 0))
                    if context.metadata
                    else "0"
                )
            else:
...
def create_full_html_document(html: str, css: str, options: Dict[str, Any]) -> str:
    ...
    <head>
        ...
        @page {{
            ...
            margin: {options.get('margins', {}).get('top', 20)}mm
                   {options.get('margins', {}).get('right', 20)}mm
                   {options.get('margins', {}).get('bottom', 20)}mm
                   {options.get('margins', {}).get('left', 20)}mm;
        }}
        </style>
    </head>
    <body>
...
def get_crop_health_template() -> str:
    ...
    <div style="font-family: 'Inter', sans-serif; max-width: 800px;
         margin: 0 auto; padding: 40px 20px; background-color: #ffffff;">
...
def get_volume_template() -> str:
    ...
    <div style="font-family: 'Inter', sans-serif; max-width: 800px;
         margin: 0 auto; padding: 40px 20px; background-color: #ffffff;">
      <header style="text-align: center; margin-bottom: 40px;
             padding-bottom: 20px; border-bottom: 3px solid #3b82f6;">
        <h1 style="font-size: 32px; font-weight: 700; color: #1e293b;
           margin-bottom: 8px;">
...
def get_area_survey_template() -> str:
    ...
    <div style="font-family: 'Inter', sans-serif; max-width: 800px;
         margin: 0 auto; padding: 40px 20px; background-color: #ffffff;">
      <header style="text-align: center; margin-bottom: 40px;
             padding-bottom: 20px; border-bottom: 3px solid #f59e0b;">
        <h1 style="font-size: 32px; font-weight: 700; color: #1e293b;
           margin-bottom: 8px;">
          Area Survey Report
        </h1>
